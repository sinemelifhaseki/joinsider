...
/*         MATCH          */
func CreateMatch(db *sql.DB, week int,team1 Team, team2 Team) (int64, error) {
...
func DistributeFixture(db *sql.DB, league_id int) error { // I configured the league fixed as 6 weeks since there are 4 teams in leagues. However it can be changed due to the expected outcome
	...
	CreateMatch(db, 3, Teams.Teams[0], Teams.Teams[3])
	CreateMatch(db, 3, Teams.Teams[2], Teams.Teams[1])
	CreateMatch(db, 4, Teams.Teams[3], Teams.Teams[0])
	CreateMatch(db, 4, Teams.Teams[0], Teams.Teams[1])
	CreateMatch(db, 5, Teams.Teams[1], Teams.Teams[3])
...
func PlayOneWeek(db *sql.DB, week int, league_id int) error {
    ...
    for rows.Next() {
		...
		goal2 := rand.Intn(5) 
		// update matches
		sql = "UPDATE matches SET is_played = 1, home_team_score = ?, away_team_score = ? WHERE home_team_id = "+strconv.Itoa(match.HomeTeamID) +" AND away_team_id = "+strconv.Itoa(match.AwayTeamID)
		stmt, err3 := db.Prepare(sql)
		if err3 != nil {
			panic(err)
		}
		defer stmt.Close()
		stmt.Exec(goal1, goal2)
		// new scores
		switch {
		case goal1 > goal2:
			sql1 := "UPDATE scores SET won = won + 1, points = points + 3, for = for + "+strconv.Itoa(goal1) +", against = against + "+strconv.Itoa(goal2)+" WHERE team_id = "+strconv.Itoa(match.HomeTeamID)
			sql2 := "UPDATE scores SET lost = lost + 1, for = for + "+strconv.Itoa(goal2)+", against = against + "+strconv.Itoa(goal1)+"WHERE team_id = "+strconv.Itoa(match.AwayTeamID)
			db.Query(sql1)
			db.Query(sql2)
		case goal2 > goal1:
			sql1 := "UPDATE scores SET won = won + 1, points = points + 3, for = for + "+strconv.Itoa(goal2)+", against = against + "+strconv.Itoa(goal1)+"WHERE team_id = "+strconv.Itoa(match.AwayTeamID)
			sql2 := "UPDATE scores SET lost = lost + 1, for = for + "+strconv.Itoa(goal1)+", against = against + "+strconv.Itoa(goal2)+"WHERE team_id = "+strconv.Itoa(match.HomeTeamID)
			db.Query(sql1)
			db.Query(sql2)
		case goal2 == goal1:
			sql1 := "UPDATE scores SET drawn = drawn + 1, points = points + 1, for = for + "+strconv.Itoa(goal2)+", against = against + "+strconv.Itoa(goal1)+"WHERE team_id = "+strconv.Itoa(match.AwayTeamID)
			sql2 := "UPDATE scores SET drawn = drawn + 1, points = points + 1, for = for + "+strconv.Itoa(goal1)+", against = against + "+strconv.Itoa(goal2)+"WHERE team_id = "+strconv.Itoa(match.HomeTeamID)
			db.Query(sql1)
			db.Query(sql2)
		}
